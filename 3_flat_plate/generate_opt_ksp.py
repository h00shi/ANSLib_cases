...
for p in pset:
    for h in hset:
        comname = 'h' + `h` + '_p' + `p`
        file_name = 'options/' + comname + '_ksp.opt'
        f = open(file_name, 'w')
        ...
        f.write('-pc_type ksp\n')
        f.write('-ksp_ksp_max_it 10\n')
        f.write('-ksp_ksp_type gmres\n')
        f.write('-ksp_ksp_norm_type preconditioned\n')
        f.write('-ksp_pc_type bjacobi\n')
        f.write('-ksp_sub_ksp_type preonly\n')
        ...
        f.write('-ita_history_name pp_data/' + comname + '_ksp.hst\n')
        f.write('-turbsa3d_pp_file pp_data/%s_ksp.postproc\n' % comname)
    
        f.write('\n# --------------------------------- ACCURACY AND STENCIL\n')
        f.write('-a ' + `p` + '\n')
        f.write('-ita_target_residual 1e-8\n')
