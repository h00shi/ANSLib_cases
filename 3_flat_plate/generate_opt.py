...
for p in pset:
    for h in hset:
        ...
        f.write('-sub_ksp_type gmres\n')
        if (h==3):  f.write('-sub_ksp_maxit 20\n')
        else:       f.write('-sub_ksp_maxit 10\n')
        f.write('-sub_ksp_norm_type preconditioned\n')
        f.write('-sub_pc_factor_levels 2\n')
        f.write('-sub_pc_factor_mat_ordering_type rcm\n')

        f.write('-ksp_gmres_preallocate\n')
        f.write('-sub_ksp_gmres_preallocate\n')
