...
for p in pset:
    ...
    f.write('-vv 1\n')
    f.write('-pc_type %s\n')
    f.write('-%s_ksp_max_it 20\n' % ksptype)
    f.write('-%s_ksp_type gmres\n' % ksptype)
    f.write('-%s_pc_type bjacobi\n' % ksptype)
    f.write('-%s_sub_pc_type ilu\n' % ksptype)
    f.write('-%s_sub_pc_factor_levels 0\n' % ksptype)
    f.write('-%s_sub_pc_factor_mat_ordering_type lines\n' % ksptype)
    ...
    f.write('\n# --------------------------------- FILE NAMES\n')
    f.write('-f   MESH/goodpart/multi \n')
    f.write('-fec MESH/goodpart/multi \n')
    f.write('-B   MESH/goodpart/multi \n')
