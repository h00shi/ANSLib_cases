...
for p in pset:
    for h in hset:
        comname = 'h' + `h` + '_p' + `p`
        file_name = 'options/' + comname + '_facet.opt'
        f = open(file_name, 'w')
        ...
        # if (p > 1):
        #     comname2 = 'h' + `h` + '_p' + `p - 1`
        #     f.write('-sol sol/' + comname2 + '_facet\n')
        f.write('-sol_out sol/' + comname + '_facet\n')
        f.write('-ita_history_name pp_data/' + comname + '_facet.hst\n')

        f.write('\n# --------------------------------- ACCURACY AND STENCIL\n')
        f.write('-a ' + `p` + '\n')
        f.write('-ita_target_residual 1e-6\n')
