...
function init
  ...
  g.fid = fopen("structured_sphere.geo", "w");
  g.ip = 1;
...
prog_r = { prog_surf{3}, 'Progression', 1.3};
s2 = write_sphere(6,    l);    #struct


rc = {'n'};


s1 = write_sphere(1,    l);    #sphere
...
# Surfaces
# surface of spheres
trans('Surface', [s1.left,s1.right,s1.front,s1.top,s1.bottom,s1.back], rc);
...
fprintf(g.fid, "%d, %d, %d, %d, %d, %d };\n", v.left, v.right, v.top, v.bottom, v.front, v.back);

fprintf(g.fid, "Physical Surface(%d) = { ", 1 ); # wall
fprintf(g.fid, "%d, %d, %d, %d, %d, %d}; \n", s1.left, s1.right, s1.top, s1.bottom, s1.front, s1.back);

fprintf(g.fid, "Physical Surface(%d) = { ", 5 ); # farfield
fprintf(g.fid, "%d, %d, %d, %d, %d, %d}; \n", s2.left, s2.right, s2.top, s2.bottom, s2.front, s2.back);
